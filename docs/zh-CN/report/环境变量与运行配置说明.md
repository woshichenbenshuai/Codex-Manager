...
- `CODEXMANAGER_WEB_BOOTSTRAP_PASSWORD`：旧版仅访问密码实例迁移到管理员账户时使用的 bootstrap 密码。Compose 示例要求显式配置它，但完成迁移后它不会作为正式登录凭据使用。
- `CODEXMANAGER_WEB_TOTP_ENCRYPTION_KEY`：验证器密钥加密密钥，必须是稳定的 32 字节 Base64 或 64 位十六进制值。service/web 分离部署时两端必须使用相同值。
- `CODEXMANAGER_WEB_PUBLIC_BASE_URL`：非 loopback Web 监听时必填，且必须是最终用户访问的 `https://` 地址；用于固定 Cookie 与 Origin/Referer 校验边界，不能用 `X-Forwarded-Proto` 替代。
- `CODEXMANAGER_WEB_TRUSTED_PROXY_CIDRS`：可选的可信反向代理 CIDR 列表（逗号分隔）。只有来自这些网段的连接才会采用 `X-Forwarded-For`；其他连接始终按 TCP 对端地址限流。
...
- `CODEXMANAGER_WEB_PUBLISH_HOST`：仅用于 Docker Compose 的宿主机 Web 发布地址。`docker/` 下的通用 Compose 默认 `127.0.0.1`；仓库根目录面向 NAS/服务器的 `17000` 入口默认 `0.0.0.0`，以保留公网访问。公网入口必须位于 HTTPS 负载均衡或反向代理之后，并通过防火墙限制来源。
...
- Web 正式登录使用账户名、密码和账户策略要求的 TOTP 验证码；旧版 `web.auth.password_hash` 只在首次迁移时校验，迁移完成后不再作为登录凭据。
- `docker/` 下的官方 Compose 默认只把 Web 端口绑定到宿主机 `127.0.0.1`；根目录 Compose 为兼容现有服务器部署，保留公网 `17000 -> 48761`。两类部署都不会发布内部 service 端口 `48760`。根目录公网入口必须由 HTTPS 负载均衡/反向代理保护，直连 service 则必须使用显式 Compose override 并自行承担访问控制。
